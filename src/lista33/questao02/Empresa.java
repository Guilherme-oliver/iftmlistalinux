...
public class Empresa {
        ...
        if (this.empregados.size() > 1) {
            for (int i = 0; i < this.empregados.size(); i++){
                if (this.empregados.get(i).getNome() == nome){
                    System.out.printf(this.empregados.get(i).getNome());
                    this.empregados.indexOf(nome);
                }
            }
        }
        return -1;
    ...
    public void demite(String nome){
        if (this.empregados.size() > 1){
            int posicao = this.empregados.indexOf(nome);
            for (int i = 0; i < this.empregados.size(); i++){
                if (this.empregados.get(i).getNome() == nome){
                    this.empregados.remove(i);
                    nome.indexOf(posicao);
                }
            }
        }
    }
